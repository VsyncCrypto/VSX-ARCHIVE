...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Vsync address: ") + input.get_str());
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the vsync address, the value is the btc amount\n"
            "      ,...\n"
            "    }\n"
    ...
    BOOST_FOREACH(const string& name_, addrList) {
        ...
        if (!address.IsValid())
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Vsync address: ")+name_);
